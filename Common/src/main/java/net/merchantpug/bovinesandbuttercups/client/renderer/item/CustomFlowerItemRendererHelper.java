...
public class CustomFlowerItemRendererHelper {
    public static void render(ItemStack stack, PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay, ItemTransforms.TransformType transformType) {
        ...
        boolean bl = !flowerModel.usesBlockLight();
        if (transformType == ItemTransforms.TransformType.GUI && bl) {
            Lighting.setupForFlatItems();
        }

        Minecraft.getInstance().getItemRenderer().render(stack, transformType, left, poseStack, bufferSource, light, overlay, flowerModel);
    }
}
