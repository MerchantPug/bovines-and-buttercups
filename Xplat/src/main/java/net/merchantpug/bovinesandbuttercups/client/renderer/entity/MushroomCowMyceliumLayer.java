...
public class MushroomCowMyceliumLayer extends RenderLayer<MushroomCow, CowModel<MushroomCow>> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int light, MushroomCow mushroomCow, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        ...
        if (Services.PLATFORM.getMushroomCowTypeFromCow(mushroomCow).getConfiguration().backGrassTinted()) {
        ...
        this.getParentModel().renderToBuffer(poseStack, buffer.getBuffer(RenderType.entityTranslucent(Services.PLATFORM.getMushroomCowTypeFromCow(mushroomCow).getConfiguration().backTexture())), light, LivingEntityRenderer.getOverlayCoords(mushroomCow, 0.0F), r, g, b, 1.0F);
    }
}
